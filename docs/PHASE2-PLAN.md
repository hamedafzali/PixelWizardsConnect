...
T7  Extract PixelWizard.Transport.WebSocket (new project, from T6's leftover)
...
| **T7 — Extract PixelWizard.Transport.WebSocket** | Lowest apparent risk (STATUS_REPORT rates `WebSocketHostServer` extraction 3/5, the lowest of the mechanical moves) but the browser-viewer protocol is unversioned and untested by anything in this repo (`docs/PROTOCOL.md`'s own words: "no browser JS changes were needed in any Phase 1 commit" — meaning also no test caught one if it had been needed). Early warning: browser viewer connects but frames don't render, with the .NET side reporting nothing wrong. | Manually verify the browser viewer end-to-end (not just `dotnet build`) before merging this task specifically, since it's the one component in the plan with no automated coverage on either side of the extraction. If that manual check fails, revert the single-project split; nothing downstream depends on `WebSocketHostServer` having its own project. |
